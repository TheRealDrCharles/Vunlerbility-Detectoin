...
class BPLSTMnet(nn.Module):
    def __init__(self, weights):
        super(BPLSTMnet, self).__init__()
        self.embedding = nn.Embedding.from_pretrained(weights, freeze=False)
        self.bplstm = BiPLSTM(100, 128).to(device)
    ...
    def forward(self, x):
        ts = torch.rand(x.shape).to(device)
        #ts = torch.rand(x.shape).to(device)
...
class BPLSTM_Feature_Net(nn.Module):
    def __init__(self, net, weights):
        super(BPLSTM_Feature_Net, self).__init__()
